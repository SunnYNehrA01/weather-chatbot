...
`weatherBOT` is a highly restricted, offline AI Weather Intelligence Platform engineered to operate entirely on isolated Edge devices (including Raspberry Pi). It leverages localized Machine Learning models (XGBoost), PyTorch Graph Neural Networks (GNN), and a local LLM (Ollama `llama3.1:8b`) to provide zero-latency, secure weather predictions without relying on real-world internet connectivity or external APIs.
...
graph TD
    ...
    subgraph Services & Engines
        ...
        MCPRouter --> ML[ML Service / XGBoost]
...
1. **Historical Data Mode**: Injects metrics from a trained XGBoost model (RMSE, Feature Importances, Sample Data). The LLM is strictly instructed to ignore any live sensor requests.
